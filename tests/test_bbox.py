...
def test_normalize_bboxes():
    ...
    bboxes = BBoxesInternalType(array=np.array(bboxes, dtype=float))
    bboxes = normalize_bboxes_np(bboxes, rows=200, cols=400)
    assert np.array_equal(bboxes.array, expected)
...
def test_normalize_denormalize_bboxes():
    ...
    bboxes_in = BBoxesInternalType(array=np.array(bboxes, dtype=float))
    bboxes_in = normalize_bboxes_np(bboxes_in, rows=200, cols=400)
    bboxes_in = denormalize_bboxes_np(bboxes_in, rows=200, cols=400)
    assert np.array_equal(bboxes_in.array, bboxes)


def test_denormalize_normalize_bboxes():
...
def test_convert_bboxes_to_internal(bboxes, source_format, expected):
    np_bboxes, targets = split_bboxes_targets(expected)
    expected = BBoxesInternalType(array=np_bboxes, targets=targets)
    image = np.ones((100, 100, 3), dtype=np.uint8)
    converted_bboxes = convert_bboxes_to_internal(
...
def test_convert_bboxes_from_internal(bboxes, target_format, expected):
    np_bboxes, targets = split_bboxes_targets(bboxes)
    bboxes = BBoxesInternalType(array=np_bboxes, targets=targets)
    image = np.ones((100, 100, 3), dtype=np.uint8)
    converted_bboxes = convert_bboxes_from_internal(
...
def test_convert_bboxes_to_albumentations_and_back(bboxes, bbox_format):
    ...
    converted_back_bboxes = convert_bboxes_from_internal(
        converted_bboxes, rows=image.shape[0], cols=image.shape[1], target_format=bbox_format
    )

    for bbox, converted_back_bbox in zip(bboxes, converted_back_bboxes):
        np.all(np.isclose(bbox, converted_back_bbox))
...
def test_random_sized_crop_size():
    ...
    np_bboxes, targets = split_bboxes_targets(bboxes)
    bboxes_internal = BBoxesInternalType(array=np_bboxes, targets=targets)
    aug = RandomSizedCrop(min_max_height=(70, 90), height=50, width=50, p=1.0)
    transformed = aug(image=image, bboxes=bboxes_internal)
